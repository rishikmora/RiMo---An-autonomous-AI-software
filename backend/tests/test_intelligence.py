...
class TestRepoSwarm:
    """Multi-repo swarm ordering — pure-logic via the toposort helper."""

    def test_toposort_orders_dependencies_first(self) -> None:
        from app.orchestration.swarm import _toposort
        ...
        order, _, cycle = _toposort(deps)
        assert not cycle
        # shared must come before backend, backend before frontend
        assert order.index("shared") < order.index("backend")
        assert order.index("backend") < order.index("frontend")

    def test_toposort_detects_cycle(self) -> None:
        from app.orchestration.swarm import _toposort

        deps = {"a": {"b"}, "b": {"a"}}
        order, _, cycle = _toposort(deps)
        assert cycle is True
        # still returns all nodes so callers can proceed best-effort
        assert set(order) == {"a", "b"}

    def test_toposort_independent_nodes(self) -> None:
        from app.orchestration.swarm import _toposort
...
class TestEconomicDecisions:
    """Founder-style ROI reject/build/defer decisions."""

    def test_rejects_costly_no_revenue_feature(self) -> None:
        from app.services.economics import economics

        d = economics.evaluate_feature_roi(
            title="Vanity feature", monthly_cost_usd=500,
            expected_monthly_revenue_usd=0, strategic_value=0.1,
        )
        assert d["decision"] == "reject"

    def test_builds_profitable_feature(self) -> None:
        from app.services.economics import economics

        d = economics.evaluate_feature_roi(
            title="Paid tier", monthly_cost_usd=200,
            expected_monthly_revenue_usd=2000, build_cost_usd=5000, confidence=0.6,
        )
        assert d["decision"] == "build"
        assert d["payback_months"] is not None

    def test_builds_strategic_even_if_unprofitable(self) -> None:
        from app.services.economics import economics

        d = economics.evaluate_feature_roi(
            title="SSO", monthly_cost_usd=100,
            expected_monthly_revenue_usd=0, strategic_value=0.8,
        )
        assert d["decision"] == "build"

    def test_defers_marginal_feature(self) -> None:
        from app.services.economics import economics

        d = economics.evaluate_feature_roi(
            title="Minor polish", monthly_cost_usd=0,
            expected_monthly_revenue_usd=0, strategic_value=0.4,
        )
