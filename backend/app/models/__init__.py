...
class Repository(Base, TimestampMixin):
    """A repository within a (possibly multi-repo) project — the swarm unit.

    A project can coordinate several repos as one system: a frontend, a backend,
    an infra repo, a mobile app, shared libraries. Each carries its role and its
    own branch/language so agents can reason about cross-repo work.
    """
    ...
    role: Mapped[RepoRole] = mapped_column(Enum(RepoRole, native_enum=False), index=True)
    full_name: Mapped[str] = mapped_column(String(512))  # owner/repo
    url: Mapped[str | None] = mapped_column(String(1024))
    default_branch: Mapped[str] = mapped_column(String(255), default="main")
    primary_language: Mapped[str | None] = mapped_column(String(64))
    # Free-form declared dependencies on other repos by role, e.g.
    # {"depends_on": ["backend", "shared"]} — used to order cross-repo work.
    meta: Mapped[dict] = mapped_column(JSONB, default=dict)

    __table_args__ = (
        UniqueConstraint("project_id", "full_name", name="uq_repo_project_fullname"),
    )
