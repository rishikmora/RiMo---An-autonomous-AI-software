...
class KnowledgeGraphService:
    ...
    async def rebuild(
        ...
        for n in nodes:
            ...
            session.add(row)
            key_to_row[n.key] = row
        await session.flush()  # assigns row.id for every node

        # Build the key->id map AFTER flush — the UUID default is applied during
        # flush, so reading row.id before this point yields None (which silently
        # dropped every edge in earlier versions).
        key_to_id: dict[str, uuid.UUID] = {k: r.id for k, r in key_to_row.items()}
    ...
    async def impact_analysis(
        ...
        *,
        ...
        """Answer "what breaks if I change this?" via transitive reverse reachability.

        Walks the dependency edges *backwards* from the target node (things that
        depend on it, things that depend on those, ...) up to ``max_depth``,
        grouped by hop distance. This is the blast radius of a change: a high
        count, or impacted API routes / DB tables, means the change is risky and
        should get extra review and benchmarking.
        """
        target = (
            await session.execute(
                select(GraphNode).where(
        ...
        if not target:
            return {"node": None, "impacted": [], "by_depth": {}, "total": 0, "risk": "unknown"}

        # Load the project's edges once and build a reverse adjacency map.
        edges = (
            await session.execute(
        ...
        for e in edges:
            reverse.setdefault(e.target_id, []).append(e.source_id)

        # BFS backwards from the target, recording the first depth each node is hit.
        depth_of: dict[uuid.UUID, int] = {}
        frontier = [target.id]
        depth = 0
        while frontier and depth < max_depth:
            ...
            for nid in frontier:
                for dependent in reverse.get(nid, []):
                    if dependent not in depth_of and dependent != target.id:
                        depth_of[dependent] = depth
                        nxt.append(dependent)
            frontier = nxt

        if not depth_of:
            return {
                "node": {"key": target.key, "name": target.name, "kind": target.kind.value},
                "impacted": [], "by_depth": {}, "total": 0, "risk": "isolated",
            }

        # Hydrate impacted node metadata.
        impacted_nodes = (
            await session.execute(
                select(GraphNode).where(GraphNode.id.in_(list(depth_of.keys())))
            )
        ).scalars().all()
        impacted = sorted(
            (
        ...
        # Risk heuristic: impacted API routes / DB tables raise severity.
        critical = sum(1 for d in impacted if d["kind"] in {"api_route", "db_table"})
        total = len(impacted)
        if critical >= 1 and total >= 10:
            risk = "high"
        elif total >= 10 or critical >= 1:
            risk = "medium"
        else:
            risk = "low"
